class AndroidsController < ApplicationController
  def create
    ...
    system("cd ~/appcreator_apps && cordova create #{folder_name} com.goodappz.#{app_name} #{app_name}")
    ...
    system("chmod 777 ~/appcreator_apps/#{folder_name}")
    ...
    Zip::ZipFile.open(icons_zip_file_data) { |zip_file|
      zip_file.each { |f|
        f_path=File.join("~/appcreator_apps/#{folder_name}/platforms/android/res", f.name)
        FileUtils.mkdir_p(File.dirname(f_path))
        zip_file.extract(f, f_path) {true}
      }
    }
 
    logger.info(system("ls ~/appcreator_apps/#{folder_name}"))
    logger.info(system("cat ~/appcreator_apps/#{folder_name}/config.xml"))
